...
- `matlab/utils` – helper utilities for registering controllers and querying simulator parameters from anywhere in the workspace.
...
3. Pick a scenario inside `runsim.m`. Each scenario bundles a trajectory preset (hover, line, diamond, circle) with a controller preset (PID or LQR) and visualization settings. Swap scenarios by updating the `activeScenario` label near the top of the script; `configureScenario` applies the bundled presets for you.
...
- **Status monitoring.** `checkStatus()` stops the simulation if the quadcopter exits the valid bounds, collides, or successfully hovers at the goal position, using the arena limits and recent motion history to decide.
